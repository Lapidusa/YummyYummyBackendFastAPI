...
class PizzaIngredient(Base):
  ...
  pizza_id: Mapped[UUID_PY] = mapped_column(PGUUID(as_uuid=True), ForeignKey("pizzas.id"), primary_key=True)
  ingredient_id: Mapped[UUID_PY] = mapped_column(PGUUID(as_uuid=True), ForeignKey("ingredients.id"), primary_key=True)
...
class BaseProductMixin:
  ...
  @declared_attr
  def variants(self) -> Mapped[List["ProductVariant"]]:
    return relationship("ProductVariant", back_populates="product", cascade="all, delete-orphan")

  @declared_attr
  def replacements(self) -> Mapped[List["ProductReplacement"]]:
...
class Pizza(Product):
  ...
  id: Mapped[UUID_PY] = mapped_column(PGUUID(as_uuid=True), ForeignKey("products.id"), primary_key=True)
